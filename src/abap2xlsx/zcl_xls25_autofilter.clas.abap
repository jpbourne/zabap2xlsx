CLASS zcl_xls25_autofilter DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      BEGIN OF ts_filter,
        ...
        t_values         TYPE HASHED TABLE OF zxls25_cell_value WITH UNIQUE KEY table_line,
    ...
    DATA filter_area TYPE zxls25_s_autofilter_area .
    ...
    METHODS get_filter_area
      RETURNING
        VALUE(rs_area) TYPE zxls25_s_autofilter_area
      RAISING
        zcx_xls25 .
    METHODS get_filter_range
      RETURNING
        VALUE(r_range) TYPE zxls25_cell_value
      RAISING
        zcx_xls25.
    METHODS get_filter_reference
      RETURNING
        VALUE(r_ref) TYPE zxls25_range_value
      RAISING
        zcx_xls25 .
    METHODS get_values
      RETURNING
        VALUE(rt_filter) TYPE zxls25_t_autofilter_values .
    METHODS is_row_hidden
      IMPORTING
        !iv_row             TYPE zxls25_cell_row
      RETURNING
        VALUE(rv_is_hidden) TYPE abap_bool .
    METHODS set_filter_area
      IMPORTING
        !is_area TYPE zxls25_s_autofilter_area .
    METHODS set_text_filter
      IMPORTING
        !i_column            TYPE zxls25_cell_column
    ...
    METHODS set_value
      IMPORTING
        !i_column TYPE zxls25_cell_column
        !i_value  TYPE zxls25_cell_value .
    METHODS set_values
      IMPORTING
        !it_values TYPE zxls25_t_autofilter_values .
...
*"* do not include other source files here!!!
  PROTECTED SECTION.
    ...
    METHODS is_row_hidden_single_values
      IMPORTING
        !iv_row             TYPE zxls25_cell_row
        !iv_col             TYPE zxls25_cell_column
...
*"* do not include other source files here!!!
  PRIVATE SECTION.
    ...
    DATA worksheet TYPE REF TO zcl_xls25_worksheet .
...
CLASS zcl_xls25_autofilter IMPLEMENTATION.
  ...
  METHOD get_filter_range.
    ...
    l_col_start_c = zcl_xls25_common=>convert_column2alpha( ip_column = filter_area-col_start ) .
    l_col_end_c   = zcl_xls25_common=>convert_column2alpha( ip_column = filter_area-col_end ) .
  ...
  METHOD get_filter_reference.
    ...
    r_ref = zcl_xls25_common=>escape_string( ip_value = l_value ).
...
*  see method documentation how to use this
  ...
  METHOD validate_area.
    DATA: l_col                   TYPE zxls25_cell_column,
          ls_original_filter_area TYPE zxls25_s_autofilter_area,
          l_row                   TYPE zxls25_cell_row.
