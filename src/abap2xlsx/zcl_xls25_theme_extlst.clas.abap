CLASS zcl_xls25_theme_extlst DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_xls25_theme_extlst IMPLEMENTATION.
  ...
  METHOD build_xml.
    ...
    IF extlst IS INITIAL.
      lo_theme_objdef ?= io_document->create_simple_element_ns( prefix = zcl_xls25_theme=>c_theme_prefix
                                                                name   = zcl_xls25_theme=>c_theme_extlst
                                                                parent = lo_theme ).
  ...
  METHOD load.
    extlst = zcl_xls25_common=>clone_ixml_with_namespaces( io_extlst ).
  ENDMETHOD.                    "load
ENDCLASS.
